...
# torch.save(model.pred_local,f'bqid_wo_train/{args.dataset}/readout/linear_{args.dataset}_{args.method}.pth')
...
for run in range(args.runs):
    ...
    for epoch in range(args.epochs):
        ...
        if result[1] > best_val:
            best_val = result[1]
            best_test = result[2]
            # best_test_quant = result[-2]
            quant = result[-2]
            if not os.path.exists(f'bqid_wo_train/{args.dataset}'):
                os.makedirs(f'bqid_wo_train/{args.dataset}')
            torch.save(quant,f"bqid_wo_train/{args.dataset}/semantic_test_{args.dataset}_{args.method}_{run}.pt")
            if not os.path.exists(f'bqid_wo_train/{args.dataset}/readout'):
                os.makedirs(f'bqid_wo_train/{args.dataset}/readout')
            torch.save(model.pred_local,f'bqid_wo_train/{args.dataset}/readout/linear_{args.dataset}_{args.method}_{run}.pth')
            if args.save_model:
                save_model(args, model, optimizer, run)
...
def save_results(args, results):
    ...
    filename = f'results/{args.dataset}/{args.method}_without_train.csv'
    print(f"Saving results to {filename}")
    with open(f"{filename}", 'a+') as write_obj:
        write_obj.write(
            f"{args.method} " + f"{args.lr} " + \
