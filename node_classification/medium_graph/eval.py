...
def evaluate(model, dataset, split_idx, eval_func, criterion, args, result=None):
    if result is not None:
        ...
        model.eval()
        out, quant = model(dataset.graph['node_feat'], dataset.graph['edge_index'])
    train_acc = eval_func(
    ...
    #     dataset.label[split_idx['test']], quant[split_idx['test']])
    ...
    return train_acc, valid_acc, test_acc, valid_loss,quant, out
...
def evaluate_cpu(model, dataset, split_idx, eval_func, criterion, args, device, result=None):
    ...
    return train_acc, valid_acc, test_acc, valid_loss, out, quant