    ...
    def print_statistics(self, run=None, mode='max_acc'):
        if run is not None:
            ...
            for r in result:
                ...
                test1 = r[:, 2].max().item()
                valid = r[:, 1].max().item()
                test_quant1 = r[:, 4].max().item()
                if mode == 'max_acc':
                    train2 = r[r[:, 1].argmax(), 0].item()
                    test2 = r[r[:, 1].argmax(), 2].item()
                    test_quant2 = r[r[:, 1].argmax(), 4].item()
                else:
                    train2 = r[r[:, 3].argmin(), 0].item()
                    test2 = r[r[:, 3].argmin(), 2].item()
                    test_quant2 = r[r[:, 3].argmin(), 4].item()
                best_results.append((train1, test1, test_quant1,valid, train2, test2,test_quant2))
            ...
            r = best_result[:, 2]
            print(f'Highest Quant Test: {r.mean():.2f} ± {r.std():.2f}')            
            r = best_result[:, 3]
            ...
            r = best_result[:, 6]
            print(f'   Final Quant Test: {r.mean():.2f} ± {r.std():.2f}')

            self.test=r.mean()
            return best_result[:, 5],best_result[:, 6]
            ...
            # if result.size(-1) ==5:
            #     r1 = best_result[:, 5]
            #     print(f'   Final Test My: {r1.mean():.2f} ± {r1.std():.2f}')
            # self.test=r.mean()
            # if result.size(-1) ==5:
            #     return best_result[:, [4,5]]
            # else:
            #     return best_result[:,4]
