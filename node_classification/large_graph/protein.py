...
from models_protein import GNN
...
def evaluate(args, model, dataloader, labels, train_idx, val_idx, test_idx, criterion, evaluator):
    ...
    embeddings = torch.zeros(labels.size(0),320).to(device)
    # quant_preds = torch.zeros(labels.shape).to(device)
    ...
    for _ in range(eval_times):
        for input_nodes, output_nodes, subgraphs in dataloader:
            ...
            pred,embedding = model(subgraphs)
            preds[output_nodes] = pred
            embeddings[output_nodes] = embedding
            # quant_preds[output_nodes] = quant_pre

            # torch.cuda.empty_cache()
    quants = encoder_decoder(embeddings.clone())
    quant_preds = model.pred_linear(quants)
    # quants /= eval_times
    # preds /= eval_times
    # quant_preds /= eval_times
    ...
    return (
        ...
        evaluator(quant_preds[test_idx], labels[test_idx]),
        train_loss,
        val_loss,
        test_loss,
        preds,
...
def run(args, graph, labels, train_idx, val_idx, test_idx, evaluator, n_running):
    ...
    total_time = 0
    val_score, best_val_score, final_test_score, final_quant_test_score = 0, 0, 0, 0
    ...
    for epoch in range(1, args.n_epochs + 1):
        ...
        if epoch == args.n_epochs or epoch % args.eval_every == 0 or epoch % args.log_every == 0:
            train_score, val_score, test_score, quant_test_score, train_loss, val_loss, test_loss, pred,quant,quant_pre = evaluate(
            ...
            if val_score > best_val_score:
                ...
                final_quant_test_score = quant_test_score
                final_pred = pred
                final_quant = quant
                final_quant_pre = quant_pre
                torch.save(final_quant, f"./save_path/protein_{n_running}.pt")
                torch.save(model.pred_linear,f"./save_path/protein_{n_running}_readout.pth")
            ...
            if epoch % args.log_every == 0:
                print(f'Epoch: {epoch:02d}, '
                  ...
                  f'Best Quant Test: {100 * final_quant_test_score:.2f}%')
            for l, e in zip(
...
def main():
    ...
    argparser.add_argument("--n-runs", type=int, default=2, help="running times")
