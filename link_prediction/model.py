...
def transfer(X):
    D = X.size(-1)
    G = torch.randn(D, D, device=X.device)
...
def bsq(x):
    ...
    x = x - mu
    xq = (x>=0)
    xq = xq.float().view(-1,d//4,4)
    basis = 2 ** torch.arange(4 - 1, -1, -1).to(device=device)
    indices = (xq * basis).sum(axis=-1)
    return indices
...
class CNLinkPredictor(nn.Module):
    ...
    def multidomainforward(self,
        ...
        # print("tar_ei.shape:", tar_ei.shape, "tar_ei.dtype:", tar_ei.dtype)
        # print("tar_ei[0]:", tar_ei[0])
        # print("tar_ei[1]:", tar_ei[1])
        # print("type(x):", type(x))
        # if isinstance(x, tuple):
        #     print("len(x):", len(x))
        #     for i, xx in enumerate(x):
        #         print(f"x[{i}].shape: {getattr(xx, 'shape', None)}, dtype: {getattr(xx, 'dtype', None)}")
