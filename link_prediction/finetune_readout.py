...
def train(h, 
    ...
    for perm in PermIterator(
        ...
        # h = feats
        edge = pos_train_edge[:, perm].to(h.device)
        # for name, param in predictor.named_parameters():
        #     print(name, param.device)
        # print("edge", edge.device)
        # print("h", h.device)
...
@torch.no_grad()
def test(h, predictor, data, split_edge, evaluator, batch_size,
...
def main():
    ...
    for run in range(0, args.runs):
        ...
        # build model
        checkpoint = torch.load(f'saved_h_predictor_{args.dataset}_{run}.pth')
        ...
        print(predictor)
        # optimizer = torch.optim.Adam([{'params': predictor.parameters(), 'lr': args.prelr, 'weight_decay': 1e-3}])
        optimizer = torch.optim.Adam([{'params': predictor.parameters(), 'lr': args.prelr}])
